...
class elFinder {
	...
	protected $commands = array(
		...
		'mkrule'     => array('target' => true, 'name' => true),
		'mkfilter'  => array('target' => true, 'name' => true),
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function mkrule($args) {
		$target = $args['target'];
		$name   = $args['name'];
		
		if (($volume = $this->volume($target)) == false) {
			return array('error' => $this->error(self::ERROR_MKRULE, $name, self::ERROR_TRGDIR_NOT_FOUND, '#'.$target));
		}

		return ($dir = $volume->mkrule($target, $name)) == false
			? array('error' => $this->error(self::ERROR_MKRULE, $name, $volume->error()))
			: array('added' => array($dir));
	}
	protected function mkfilter($args) {
		$target = $args['target'];
		$name   = $args['name'];
