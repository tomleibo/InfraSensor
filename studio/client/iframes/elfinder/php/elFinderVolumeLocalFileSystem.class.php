...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver {
	...
	protected function configure() {
		...
		if (!empty($this->options['quarantine'])) {
			...
			if ((!is_dir($this->quarantine) && !$this->_mkrule($this->root, $this->options['quarantine'])) || !is_writable($this->quarantine)) {
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function _mkrule($path, $name) {
	...
	protected function _extract($path, $arc) {
		...
		if ($this->quarantine) {
			...
			if (count($ls) == 1) {
				...
				if (!$this->_mkrule(dirname($path), $name) || !copy($path, $archive)) {
					return false;
				}
