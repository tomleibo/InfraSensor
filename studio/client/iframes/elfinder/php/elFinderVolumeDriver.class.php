...
abstract class elFinderVolumeDriver {
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	public function mkrule($dst, $name) {
		if ($this->commandDisabled('mkrule')) {
		...
		$this->clearcache();
		return ($path = $this->_mkrule($path, $name)) ? $this->stat($path) : false;
	}
	
	public function mkfilter($dst, $name) {
		if ($this->commandDisabled('mkrule')) {
			return $this->setError(elFinder::ERROR_PERM_DENIED);
		}
	...
	protected function copy($src, $dst, $name) {
		...
		if ($srcStat['mime'] == 'directory') {
			...
			if (($test && $test['mime'] != 'directory') || !$this->_mkrule($dst, $name)) {
	...
	protected function copyFrom($volume, $src, $destination, $name) {
		...
		if ($source['mime'] == 'directory') {
			...
			$this->clearcache();
			if ((!$stat || $stat['mime'] != 'directory') && !$this->_mkrule($destination, $name)) {
	...
	**/
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _mkrule($path, $name);
